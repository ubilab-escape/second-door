...
MAX7221::MAX7221(uint8_t csPin, uint8_t numberOfDevices, hardwareType_t hardwareType) {
    cs_pin_ = csPin;
    number_of_devices_ = numberOfDevices;
    hardware_type_ = hardwareType;

    cols = new uint8_t[number_of_devices_ * 8];
}
...
void MAX7221::setPixel(uint8_t x, uint8_t y) {
    bitWrite(cols[x], y, true);
}

void MAX7221::setColumn(uint8_t column, uint8_t value) {
    if (column < 0 || column >= number_of_devices_ * 8) {
        return;
    }
    cols[column] = value;
}
...
void MAX7221::clear() {
    for (uint8_t i = 0; i < number_of_devices_ * 8; i++) {
        cols[i] = 0;
    }
}
